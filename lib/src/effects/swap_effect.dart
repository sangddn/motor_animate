...
/// ```
/// foo.animate()
///   .fadeOut(motion: Motion.linear(500.ms))
///   .swap( // inherits motion from fadeOut
...
/// [AnimateController]. So, for example, repeating the first animation (the
...
extension SwapEffectExtensions<T extends AnimateManager<T>> on T {
  /// Adds a [SwapEffect] that swaps out the incoming child for a new child at a particular
  /// point in time. This includes all preceding effects.
  T swap({
    Duration? delay,
    Motion? motion,
    required TransitionBuilder builder,
  }) =>
      addEffect(SwapEffect(delay: delay, motion: motion, builder: builder));
}
